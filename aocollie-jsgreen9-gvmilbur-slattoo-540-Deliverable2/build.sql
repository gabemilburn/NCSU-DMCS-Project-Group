...
FOR EACH ROW
BEGIN
    DECLARE v_old_total_oz FLOAT;
    DECLARE v_new_total_oz FLOAT;
    DECLARE v_pack_size FLOAT;
    
    IF NOW() > OLD.ExpirationDate THEN
        -- Get pack size
        SELECT PackSize INTO v_pack_size
        FROM Formulation
        WHERE FormulationID = OLD.FormulationID;
        
        SET v_old_total_oz = OLD.Quantity * v_pack_size;
        SET v_new_total_oz = NEW.Quantity * v_pack_size;
        
        IF v_new_total_oz <> v_old_total_oz THEN
...
BEGIN
    ...
    DECLARE v_new_quantity FLOAT;
    
    SELECT f.PackSize, ib.Quantity 
    INTO v_pack_size, v_current_quantity
    FROM IngredientBatch ib
    JOIN Formulation f ON ib.FormulationID = f.FormulationID
    WHERE ib.LotID = NEW.IngredientLotID;
    
    SET v_new_quantity = ((v_current_quantity * v_pack_size) - NEW.QuantityUsed) / v_pack_size;
...
DROP PROCEDURE IF EXISTS sp_view_supplier_ingredient_batches$$
CREATE PROCEDURE sp_view_supplier_ingredient_batches(
    IN p_supplier_id INT,
    IN p_include_expired BOOLEAN
)
BEGIN
    SELECT 
        ib.LotID,
        ...
        (ib.Quantity * f.PackSize) AS TotalQuantityOz,  -- CALCULATED
        ib.ExpirationDate,
        CASE 
    ...
    AND (p_include_expired = TRUE OR ib.ExpirationDate >= CURDATE())
    ORDER BY 
        CASE 
...
DROP PROCEDURE IF EXISTS sp_view_supplier_ingredients$$
CREATE PROCEDURE sp_view_supplier_ingredients(
    IN p_supplier_id INT
)
BEGIN
    SELECT DISTINCT 
...
DROP PROCEDURE IF EXISTS sp_get_active_formulations_for_supplier$$
CREATE PROCEDURE sp_get_active_formulations_for_supplier(
    IN p_supplier_id INT
)
BEGIN
    SELECT 
...
DROP PROCEDURE IF EXISTS sp_list_supplier_formulations$$
CREATE PROCEDURE sp_list_supplier_formulations(
    IN p_supplier_id INT
)
BEGIN
    SELECT 
...
BEGIN
    SELECT 
        i.IngredientID,
        i.IngredientName,
        COALESCE(SUM(ib.Quantity * f.PackSize), 0) AS TotalOnHandOz,  -- CALCULATED
    ...
    LEFT JOIN IngredientBatch ib ON ib.ManufacturerID = p_manufacturer_id
       AND ib.ExpirationDate >= CURDATE()
    LEFT JOIN Formulation f ON ib.FormulationID = f.FormulationID
...
DROP PROCEDURE IF EXISTS sp_get_available_batches_fefo$$
CREATE PROCEDURE sp_get_available_batches_fefo(
    IN p_ingredient_id INT,
    IN p_manufacturer_id INT
)
BEGIN
...
DROP PROCEDURE IF EXISTS sp_validate_recipe_inventory$$
CREATE PROCEDURE sp_validate_recipe_inventory(
    IN p_recipe_id INT,
...
BEGIN
    SELECT 
        rb.IngredientID,
        i.IngredientName,
        rb.Quantity * p_batch_quantity AS RequiredOz,
        COALESCE(SUM(ib.Quantity * f.PackSize), 0) AS AvailableOz,  -- FIXED: Calculate instead of direct reference
        CASE 
            WHEN COALESCE(SUM(ib.Quantity * f.PackSize), 0) >= rb.Quantity * p_batch_quantity 
            THEN 'SUFFICIENT'
            ELSE 'INSUFFICIENT'
        END AS Status
    FROM RecipeBOM rb
    ...
    LEFT JOIN Formulation f ON ib.FormulationID = f.FormulationID
        AND f.IngredientID = i.IngredientID
    WHERE rb.RecipeID = p_recipe_id
    GROUP BY rb.IngredientID, i.IngredientName, rb.Quantity
    HAVING Status = 'INSUFFICIENT';
END$$

DROP PROCEDURE IF EXISTS sp_get_recipe_requirements$$
CREATE PROCEDURE sp_get_recipe_requirements(
    IN p_recipe_id INT
)
BEGIN
    SELECT 
...
DROP PROCEDURE IF EXISTS sp_get_available_ingredient_batches_for_manufacturer$$
CREATE PROCEDURE sp_get_available_ingredient_batches_for_manufacturer(
    IN p_manufacturer_id INT
)
BEGIN
    SELECT 
...
DROP PROCEDURE IF EXISTS sp_get_recipes_for_product$$
CREATE PROCEDURE sp_get_recipes_for_product(
    IN p_product_id INT,
    IN p_manufacturer_id INT
)
BEGIN
    ...
    IF NOT EXISTS (
        ...
        AND ManufacturerID = p_manufacturer_id
    ) THEN
        SELECT 'Product not found or not owned by you' AS ErrorMessage;
    ELSE
...
DROP PROCEDURE IF EXISTS sp_get_product_info$$
...
DROP PROCEDURE IF EXISTS sp_list_all_product_batches$$
CREATE PROCEDURE sp_list_all_product_batches()
BEGIN
    SELECT 
        pb.LotID,
        p.ProductID,
...
CREATE PROCEDURE sp_get_batch_info(
    IN p_lot_id VARCHAR(255)
)
BEGIN
    SELECT 
        pb.LotID,
