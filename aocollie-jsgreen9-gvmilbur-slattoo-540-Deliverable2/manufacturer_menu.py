...
class ManufacturerMenu:
    ...
    def _select_product(self):
        ...
        try:
            self.cursor.callproc('sp_get_product_info', [pid, self.manufacturer_id])
    ...
    def view_recipes_for_product(self):
        ...
        try:
            self.cursor.callproc('sp_get_recipes_for_product', [pid, self.manufacturer_id])
    ...
    def view_recipe_details(self):
        ...
        try:
            self.cursor.callproc('sp_get_recipe_details', [recipe_id, self.manufacturer_id])
    ...
    def allocate_ingredients_fefo(self, recipe_id, batch_quantity):
        try:
            self.cursor.callproc('sp_get_recipe_requirements', [recipe_id])
            for result in self.cursor.stored_results():
                requirements = result.fetchall()
        except mysql.connector.Error as err:
            return (False, None, 0, f"Database error: {err}")
        ...
        for ing_id, ing_name, qty_per_unit in requirements:
            ...
            try:
                self.cursor.callproc('sp_get_available_batches_fefo', 
            ...
            if not available_batches:
                return (False, None, 0, f"No available inventory for {ing_name}")
            
            batch_found = False
            for lot_id, available_oz, unit_price, pack_size, exp_date, days_left in available_batches:
                available_oz = float(available_oz)
                unit_price = float(unit_price)
                pack_size = float(pack_size)
                
                if available_oz >= needed_oz:
                    qty_to_use = needed_oz
                    cost = (qty_to_use / pack_size) * unit_price
                    allocations.append((lot_id, qty_to_use, cost))
                    total_cost += cost
                    batch_found = True
                    break
            ...
            if not batch_found:
                return (False, None, 0, 
                    f"No single batch has sufficient quantity for {ing_name}. Need {needed_oz:.2f} oz, largest available batch has {max(float(b[1]) for b in available_batches):.2f} oz.")
        
        return (True, allocations, total_cost, "Success")

    def allocate_ingredients_manual(self, recipe_id, batch_quantity):
        try:
            self.cursor.callproc('sp_get_recipe_requirements', [recipe_id])
            for result in self.cursor.stored_results():
                requirements = result.fetchall()
        ...
        print("\n" + "="*70)
        print("MANUAL BATCH SELECTION")
        print("="*70)
        
        for ing_id, ing_name, qty_per_unit in requirements:
            needed_oz = float(qty_per_unit) * batch_quantity
            
            print(f"\n--- Ingredient: {ing_name} (ID: {ing_id}) ---")
            print(f"Required: {needed_oz:.2f} oz")
            
            try:
                self.cursor.callproc('sp_get_available_batches_fefo', 
                                    [ing_id, self.manufacturer_id])
                for result in self.cursor.stored_results():
                    all_batches = list(result.fetchall())
            except mysql.connector.Error as err:
                return (False, None, 0, f"Database error: {err}")
            ...
            sufficient_batches = [b for b in all_batches if float(b[1]) >= needed_oz]
            
            if not sufficient_batches:
                return (False, None, 0, 
                    f"No single batch has sufficient quantity for {ing_name}. Need {needed_oz:.2f} oz, largest available batch has {max(float(b[1]) for b in all_batches):.2f} oz.")
            
            print(f"\nBatches with sufficient quantity (≥{needed_oz:.2f} oz):")
            print(f"{'#':<4} {'Lot ID':<20} {'Available (oz)':<15} {'$/Pack':<10} {'Pack Size':<12} {'Expires':<12} {'Days Left':<10}")
            print("-"*95)
            for idx, batch in enumerate(sufficient_batches, 1):
                print(f"{idx:<4} {batch[0]:<20} {batch[1]:<15.2f} ${batch[2]:<9.2f} {batch[3]:<12.2f} {str(batch[4]):<12} {batch[5]:<10}")
            
            while True:
                try:
                    batch_choice = int(input("\nEnter batch # to use (or 0 to cancel): ").strip())
                except ValueError:
                    print("Error: Invalid input.")
                    continue
                
                if batch_choice == 0:
                    return (False, None, 0, "Cancelled by user")
                
                if batch_choice < 1 or batch_choice > len(sufficient_batches):
                    print(f"Error: Please enter a number between 1 and {len(sufficient_batches)}.")
                    continue
                
                selected_batch = sufficient_batches[batch_choice - 1]
                lot_id, available_oz, unit_price, pack_size, exp_date, days_left = selected_batch
                ...
                qty_to_use = needed_oz
                remainder = available_oz - needed_oz
                
                print(f"Using {qty_to_use:.2f} oz from batch (leaving {remainder:.2f} oz remaining)")
    ...
    def create_product_batch(self):
        ...
        print("\n" + "-"*60)
        print("INGREDIENT ALLOCATION METHOD")
        print("-"*60)
        print("1) FEFO (Automatic - First Expired, First Out)")
        print("2) Manual Selection (Choose specific batches)")
        print("-"*60)
        
        try:
            allocation_choice = int(input("\nSelection: "))
        except ValueError:
            print("Invalid input.")
            return
        
        if allocation_choice == 1:
            print("\nAllocating ingredients using FEFO...")
            success, allocations, total_cost, message = self.allocate_ingredients_fefo(
                recipe_id, batch_qty
            )
        elif allocation_choice == 2:
            success, allocations, total_cost, message = self.allocate_ingredients_manual(
                recipe_id, batch_qty
            )
        else:
            print("Invalid choice.")
        ...
        try:
            ...
            print("\n✓ No health risk violations detected - safe to proceed")
    ...
    def receive_ingredient_batches(self):
        ...
        try:
            self.cursor.callproc('sp_get_available_ingredient_batches_for_manufacturer', 
                                [self.manufacturer_id])
