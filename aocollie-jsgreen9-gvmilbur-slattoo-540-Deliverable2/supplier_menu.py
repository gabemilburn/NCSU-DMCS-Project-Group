...
class SupplierMenu:
    ...
    def view_formulation_details(self):
        ...
        try:
            self.cursor.callproc('sp_list_supplier_formulations', [self.supplier_id])
    ...
    def create_ingredient_batch(self):
        ...
        try:
            self.cursor.callproc('sp_get_active_formulations_for_supplier', [self.supplier_id])
            ...
            self.cursor.execute("""
                INSERT INTO IngredientBatch (
                    FormulationID, Quantity, ExpirationDate
                ) VALUES (%s, %s, %s)
            """, (formulation_id, quantity, exp_date))
    ...
    def view_ingredient_batches(self):
        ...
        try:
            self.cursor.callproc('sp_view_supplier_ingredient_batches', 
                                [self.supplier_id, include_expired])
            ...
            if results:
                ...
                if expiring_soon_count > 0:
                    print(f"\nAlert: {expiring_soon_count} batch(es) expiring within 30 days.")
            else:
    ...
    def view_my_ingredients(self):
        ...
        try:
            self.cursor.callproc('sp_view_supplier_ingredients', [self.supplier_id])
